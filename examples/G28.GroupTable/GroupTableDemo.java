...
public class GroupTableDemo extends AbstractDemo {
    ...
    public Component getOptionsPanel() {
        ...
        final JCheckBox showGroupColumns = new JCheckBox("Show Group Columns");
        ...
        JCheckBox summaryMode = new JCheckBox("Show Summary on Group Rows");
        summaryMode.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                _groupTableModel.setSummaryMode(e.getStateChange() == ItemEvent.SELECTED);
                if (_groupTableModel.isSummaryMode()) {
                    showGroupColumns.setSelected(true);
                    showGroupColumns.setEnabled(false);
                }
                else {
                    showGroupColumns.setEnabled(true);
                }
                _groupTableModel.groupAndRefresh();
                _table.expandAll();
            }
        });
        summaryMode.setSelected(_groupTableModel.isSummaryMode());
        ...
        JCheckBox hideOriginalRows = new JCheckBox("Hide Original Rows When Grouped");
        hideOriginalRows.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                _groupTableModel.setOriginalRowsHidden(e.getStateChange() == ItemEvent.SELECTED);
                _groupTableModel.groupAndRefresh();
            }
        });
        hideOriginalRows.setSelected(_groupTableModel.isOriginalRowsHidden());

        checkBoxPanel.add(singleLevel);
        checkBoxPanel.add(keepColumnOrder);
        checkBoxPanel.add(showGroupColumns);
        checkBoxPanel.add(summaryMode);
        ...
        checkBoxPanel.add(hideOriginalRows);
    ...
    public Component getDemoPanel() {
        ...
        _groupTableModel.setSummaryMode(true);

        _groupTableModel.groupAndRefresh();

        // Commented code is to show you how to use getPaddingBackground
...
//        panel.add(field, BorderLayout.AFTER_LINE_ENDS);
        ...
        panel.add(new JScrollPane(new SortableTable(_tableModel)));
        return panel;
    }
