...
//    	UIManager.getDefaults().put("CommandBar.background", new Color(61, 90, 156));
...
//        LoggerUtils.enableLogger("java.awt.focus", Level.ALL);
        ...
        theme.setBaseColor(new Color(80,105,161), true, "default");
        ...
        ThemePainter tp = (ThemePainter) UIDefaultsLookup.get("Theme.painter");
        System.out.println(tp);
        
        UIDefaultsLookup.put(UIManager.getDefaults(), "Theme.painter", new MyOffice2007Painter() {
    
        });   

        _frame = new JFrame(MessageFormat.format(Resource.RB.getString("Demo.title"), Resource.RB.getString("Demo.allProducts"), Lm.getProductVersion(), SystemInfo.getJavaVersion())) {
            /**
            ...
            public JMenuBar getJMenuBar() {
                if (_holder.getDockableBarManager() != null) {
                    Collection<DockableBar> barCollection = _holder.getDockableBarManager().getAllDockableBars();
    ...
    private static StatusBar createStatusBar() {
        ...
        progress.setCancelCallback(new ProgressStatusBarItem.CancelCallback() {
            public void cancelPerformed() {
                progress.setStatus(Resource.RB.getString("Demo.cancelled"));
                progress.showStatus();
            }
        });
            ...
            documentComponent.addDocumentComponentListener(new DocumentComponentAdapter() {
                ...
                public void documentComponentActivated(final DocumentComponentEvent e) {
                    ...
                    if (panes != null) {
                        ...
                        frame.setTitle(Resource.RB.getString("Demo.options") + " - " + demo.getName());
                        if (frame.isHidden()) {
                            getDockingManager().showFrame(OPTIONS_DOCKABLE_FRAME_KEY, false);
                        }
                    }
                    else {
                        frame.setTitle(Resource.RB.getString("Demo.options"));
                        getDockingManager().hideFrame(OPTIONS_DOCKABLE_FRAME_KEY);
                    }
