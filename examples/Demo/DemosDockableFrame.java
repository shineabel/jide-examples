/*
 * @(#)DemosDockableFrame.java 2/5/2012
 *
...
class DemosDockableFrame extends DockableFrame {
    ...
    public Color getTabBackground() {
        ...
        if (parent != null && ((JideTabbedPane) parent).getSelectedComponent() == this) return Color.RED;
        return Color.GREEN;
    }
...
//            "AutoFilterTableHeaderWithInputDemo", // remove this demo because it doesn't use the native header
...
//            "HibernatePageTableModelDemo",
    ...
    public DemosDockableFrame(String key, JideDemos demos) {
        ...
        setTitle(Resource.RB.getString("Demo.demo"));
        _demoHolder = demos;
    ...
    protected JComponent createToolBar() {
        ...
        categoryViewButton.setToolTipText(Resource.RB.getString("Demo.sortCategorized"));
        ...
        alphabetViewButton.setToolTipText(Resource.RB.getString("Demo.sortAlphabetically"));
    ...
    protected void initComponents() {
        ...
        _field.setHintText(Resource.RB.getString("Demo.filter"));
    ...
    private TreeModel reloadTreeModel(int order) {
        ComparableTreeNode root = new ComparableTreeNode(Resource.RB.getString("Demo.allProducts"));
        if (order == 0) {
            ComparableTreeNode nodeCommon = new ComparableTreeNode(ProductNames.PRODUCT_NAME_COMMON + Resource.RB.getString("Demo.openSource"));
...
//                    }
            ...
            for (Object demo : _demos) {
                ComparableTreeNode node = (ComparableTreeNode) demo;
                root.add(node);
            }
        }
    ...
    private void loadDemos() {
        int loaded = 0;
        _demos.clear();
        for (String demoClassName : DEMOS) {
            try {
                Class<?> demoClass = getClass().getClassLoader().loadClass(demoClassName);
            ...
            ((ProgressStatusBarItem) JideDemos.getStatusBar().getItemByName("Status")).setStatus(
                    loaded == DEMOS.length ? MessageFormat.format(Resource.RB.getString("Demo.allDemoLoaded"), loaded) :
                            MessageFormat.format(Resource.RB.getString("Demo.demoLoaded"), DEMOS.length, loaded));
        }
    }
